...
# screeplot으로 확인
# 적정 개수의 주성분을 선택할 수 있다. 주성분의 개수가 증가함에 따라 그 주성분에 의한 급경사가 나타나고 이후에 완만한 경사가 나타나기 때문에 기울기가 큰 부분일수록 많은 설명력을 보여준다.
...
round(scale(state.x77) %*% pca$rotation, 3) # 표준화된 전체 데이터 행렬과 성분적재값의 곱

# prcomp 함수는 각 케이스에 대한 성분점수를 계산해서 주성분개체의 X원소의 행렬 형태로 제공함
round(pca$x, 3) # 위의 결과와 같음 

# 성분점수가 큰 일부 열들만 추출도 가능. 이렇게 사용된 성분점수는 원래의 변수값 대신에 사용할 수 있고, 다수의 변수 대신에 소수의 주성분을 사용함으로써 데이터의 복잡성을 줄일 수 있다.
# PC1, PC2의 성분점수만 추출
round(pca$x[, c(1,2)], 3)
