---
title: "Line Chare_inter(변수 간 관계 파악)"
author: "PYB"
date: '2021 1 10 '
...
### 2. smoking 데이터 준비
t() 전치 함수를 사용하면 행렬(matrix) 클래스로 바뀌어 $ 연산자를 사용할 수 없다.
따라서 tibble 함수를 이용한다.

tibble: data.frame을 현대적으로 재구성한 것으로 Simple Data Frames이라고 생각할 수 있다.
tibbles은 data.frame을 만드는 좋은 방법이며, 데이터를 간략하게 표현이 가능하여 대용량 데이터 세트를 다루는데 용이하다.
```{r}
# 데이터 불러오기
...
smoking <- as_tibble(t(raw_smoking), rownames = "date") 
head(smoking)
class(smoking)
str(smoking)
...
dis$date <- as.Date(dis$date, format = "%Y", origin = '')
str(dis)
...
시계열 그래프를 만들려면 데이터가 시간 순서 속성을 지니는 xts 타입으로 되어 이어야 한다.
바꿀 변수와 기준이 되는 변수의 길이(행 개수)는 서로 같아야 한다.
dyRangeSelector는 날짜 범위를 선택할 수 있는 기능이 있다.
```{r}
...
걷기 실천자의 데이터는 19년까지 있고, 비만인 데이터는 18년까지 있으므로
데이터를 동일하게 해주기 위해 걷기 실천자의 19년 데이터를 제거한다.
```{r}
...
스트레스 인지하는 사람수 데이터는 19년까지 있고, 비만인 데이터는 18년까지 있으므로
데이터를 동일하게 해주기 위해 스트레스 인지하는 사람수의 19년 데이터를 제거한다.
```{r}
stress_a <- stress[-12, ]
...
cor.test(stress_a$sum, fat$sum) # 상관분석
```
...
cor.test(stress$sum, dis$sum) # 상관분석
```
