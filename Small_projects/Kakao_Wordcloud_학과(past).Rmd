...
## 1.준비 작업
```{r cars}
library(KoNLP) # KoNLP 로드
library(wordcloud2) # 워드 클라우드2 패키지 로드
library(ggplot2) # ggplot2 패키지 로드
library(dplyr) # dplyr 패키지 로드
library(RColorBrewer) # 글자색 표현 패키지 로드
useNIADic() # 사전 로드
```
...
## 7.글자수 범위 설정
nchar(): 각 요소 글자의 길이 알 수 있음
```{r}
text115 <- text114[nchar(text114) < 5] # 글자수 5 미만
text115 <- text115[nchar(text115) > 1] # 글자수 1 초과
```

## 8. 내림차순 정렬
sotr(): 정렬 함수(기본적으로 오름차순 정렬)
table(): 빈도수 확인
우리는 많이 사용된 단어들을 기준으로 볼 것이므로 decreasing을 통해 내림차순 한다.
```{r}
text116 <- sort(table(text115), decreasing = T)
```

## 9. 상위 50개 데이터 추출
head 함수를 이용해 상위 50개의 단어만 추출한다.
```{r}
text117 <- head(text116, 80)

#빈도수 막대 그래프로 확인하기
df <- as.data.frame(text117) 
df <- rename(df,
             word = text115)
...
## 10. 워드 클라우드 생성
난수를 설정하여 항상 동일한 워드 클라우드를 생성하도록 한다.
...
wordcloud2(text117, size = 0.7, fontFamily = "Tmon몬소리", color = pal)
```
